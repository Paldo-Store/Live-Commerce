...
- Redis 실패 시 로그 경보 (기존 `PaymentExpirationListener`가 PENDING → FAILED fallback 역할 유지)
...
Spring Boot `TransactionAutoConfiguration`이 `TransactionTemplate` Bean을 자동 등록하므로 별도 `@Configuration` 불필요. 생성자 주입만으로 사용 가능.
...
| 파일 | 설명 |
|------|------|
| `application/port/PaymentCachePort.java` | Redis 캐시 Port 인터페이스 |
| `infrastructure/redis/PaymentCacheAdapter.java` | `PaymentCachePort` 구현체 — `EXPIRE_KEY_PREFIX` 상수 및 `expire-minutes` 설정 내부 캡슐화 |
...
| `application/exception/PaymentExceptionCode.java` | `PAYMENT_EXPIRED` 추가 |
| `application/service/PaymentServiceV2.java` | `PaymentCachePort` 주입, Redis 검증/삭제 Port 위임, approvePayment 트랜잭션 분리 |
| `application/service/PaymentService.java` | `PaymentCachePort` 주입, Redis set Port 위임 |
| `infrastructure/listener/PaymentExpirationListener.java` | `PaymentCachePort` 주입, key 판별·파싱 Port 위임, `payment.fail()` 사용 |
| `infrastructure/kafka/listener/PaymentDomainEventListener.java` | `PaymentCachePort` 주입, `onPaymentReady` 핸들러 추가 |
